...
--                 ((p.STATE = N'Active') AND (a.STATE = N'Released')) THEN N'已办'
...
--                CASE
--                   WHEN (p.STATE = N'Closed') OR
--                 ((p.STATE = N'Active') AND (a.STATE = N'Released')) THEN '1'
--                  ELSE '0'
--                END,
       a.CLASSIFICATION,
       a.KEYED_NAME,
      ...
      WHERE (rollup_lead_identity IS NOT NULL)
        AND (NOT (id IN
          ...
          SELECT tmp_aa.id,
                 ...
                 IIF(tmp_aa.STATE = 'Released', tmp_aa.STATE, tmp_a.STATE)                                  AS STATE,
                 IIF(tmp_aa.STATE = 'Released', tmp_aa.CURRENT_STATE, tmp_a.CURRENT_STATE) AS CURRENT_STATE
