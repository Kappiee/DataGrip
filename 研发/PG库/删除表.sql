DROP TABLE IF EXISTS innovator.test1;
...
BEGIN
    IF EXISTS (
        SELECT 1
        FROM information_schema.tables
        WHERE table_name = 'project_activity_task'
          AND table_type = 'BASE TABLE'
    ) THEN
        EXECUTE 'DROP TABLE project_activity_task';
    END IF;
END $$;

create or replace view innovator.project_activity_task
            (id, assigned_to, item, item_type_id, formname, start_date, due_date, instructions, my_assignment, name,
             container, container_type_id, status, classification, keyed_name, created_on, created_by_id, owned_by_id,
             managed_by_id, modified_on, modified_by_id, current_state, state, locked_by_id, is_current, major_rev,
             minor_rev, is_released, not_lockable, css, generation, new_version, config_id, permission_id, team_id,
             icon, language_code_filter)
as
SELECT aid.id,
